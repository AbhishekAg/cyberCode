...
/*
	printf("inserting %d Color %s parent Color %s %u\n",
			new_node_ptr->value , 
			COLOR(new_node_ptr), 
			COLOR(new_node_ptr->parent_ptr) ,
			new_node_ptr->parent_ptr-> rb

		); 
 */
	if ( new_node_ptr->parent_ptr-> rb == RED )
	{ 
		printf("Imbalance inserting %d:",new_node_ptr->value ); 

		if ( new_node_ptr->parent_ptr == 
				new_node_ptr->parent_ptr->parent_ptr->left_child_ptr)
		{    //L_
			if ( new_node_ptr == new_node_ptr->parent_ptr->left_child_ptr)
			...
			printf("%s", COLOR(new_node_ptr->parent_ptr
